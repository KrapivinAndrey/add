...
Процедура ЗагрузитьНастройкиИзФайла(Команда)
	...
	ДиалогВыбора.Заголовок = "Выберите файл настройки xUnitFor1C";
...
Процедура ЗагрузитьПользовательскиеПлагины()
	...
	Если НЕ ПлагинНастроек.ЕстьНастройка("КаталогиПлагинов") Тогда
		
		Возврат;
		
	КонецЕсли;
		
	КаталогиПлагинов = ПлагинНастроек.ПолучитьНастройку("КаталогиПлагинов");
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("КаталогиПлагинов",     КаталогиПлагинов);
...
Процедура ДиспетчерЭкспортаОтчетовАсинхронныйПакетныйРежим(Знач РезультатЭкспорта, Знач ПараметрыДиспетчера) Экспорт
	...
	Если ПараметрыДиспетчера.ИндексВидаОтчета < СписокОтчетов.Количество() Тогда
		...
		Иначе
			ВыполнитьОбработкуОповещения(ОбработчикЭкспорта);
		КонецЕсли;
...
Процедура ПервичнаяНастройка()
	...
	ВывестиСообщение(ПолучитьНаименованиеИВерсиюОбработкиКлиент(), СтатусСообщения.ОченьВажное);
