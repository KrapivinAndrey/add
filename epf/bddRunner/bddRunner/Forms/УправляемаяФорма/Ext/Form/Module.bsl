...
&НаКлиенте
Процедура КомандаСохранитьПлагины(Команда)
	
	КомандаСохранитьНастройки(Команда);
	ЗагрузитьПользовательскиеПлагины();
...
&НаКлиенте
Процедура ДобавитьСтрокуВСписокПлагинов(Команда)
	Элементы.КаталогиПлагинов.ДобавитьСтроку();
КонецПроцедуры

&НаКлиенте
Процедура ОчиститьСтрокиВСпискеПлагинов(Команда)
	Объект.КаталогиПлагинов.Очистить();
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	...
	Объект.ИспользоватьПрямыеПутиФайлов = Ложь;//TODO удалить временную строку
...
&НаКлиенте
Процедура КаталогиПлагиновЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;

	ТекущиеДанные = Элементы.КаталогиПлагинов.ТекущиеДанные;

	ДиалогВыбораКаталога = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.ВыборКаталога);

	ДиалогВыбораКаталога.МножественныйВыбор = Ложь;

	ДиалогВыбораКаталога.Показать(Вычислить("Новый ОписаниеОповещения(""КаталогиПлагиновЗначениеНачалоВыбораЗавершение"", ЭтаФорма, ТекущиеДанные)"));

КонецПроцедуры

&НаКлиенте
Процедура КаталогиПлагиновЗначениеНачалоВыбораЗавершение(ВыбранныеФайлы, ТекущиеДанные) Экспорт
	...
	Для Каждого Строка Из Объект.КаталогиПлагинов Цикл
		
		Если Строка.Значение = Элементы.КаталогиПлагинов.ТекущиеДанные.Значение Тогда
			
			Строка.Значение = ВыбранныеФайлы[0];
			СохранитьНастройкиВХранилище("КаталогиПлагинов", Объект.КаталогиПлагинов);
			Прервать;
...
Функция ПодключитьВнешнююОбработкуКлиент(Знач ИмяФайла) Экспорт
	...
	Попытка
		...
		Если ЕстьПоддержкаНемодальныхФорм Тогда
			Оповещение = Вычислить("Новый ОписаниеОповещения(""ОбработкаПослеПомещенияФайла"",
...
Функция ПолучитьМассивФичаФайловИзКаталогаСогласноНастроек(КаталогСФичами) Экспорт
	...
	СтруктураПараметров.Вставить("КаталогиПлагинов",Объект.КаталогиПлагинов);
	...
	Иначе
		...
		Если ПомещаемыеФайлы.Количество() Тогда
			Если Версия836ИлиВыше Тогда
				...
				ПараметрыЗавершения.Вставить("ОбработчикЗавершения",   ДополнительныеПараметры.ОбработчикЗавершения);
				ПараметрыЗавершения.Вставить("КаталогиПлагинов",       ДополнительныеПараметры.КаталогиПлагинов);
				...
				// из-за асинхронности никакой код вызывать нельзя
			Иначе
				ПомещенныеФайлы = Новый Массив;
...
Процедура ЗагрузкаПлагиновЗавершение(Знач ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.КаталогиПлагинов.Удалить(0);
	
	Если ДополнительныеПараметры.КаталогиПлагинов.Количество() = 0 Тогда
	...
	Иначе
		...
		Если Версия836ИлиВыше Тогда
			...
			ОбработчикПродолжения = Вычислить("Новый ОписаниеОповещения(""ЗагрузитьПлагиныИзКаталогов"", ЭтаФорма, ДополнительныеПараметры)");
			ВыполнитьОбработкуОповещения(ОбработчикПродолжения);
			
		Иначе
			
			ЗагрузитьПлагиныИзКаталогов(ДополнительныеПараметры);
...
Функция ПодключитьВнешнююОбработкуСервер(Знач ПутьФайла, Знач АдресХранилища, Знач ИспользуетсяЗащитаОтОпасныхДействий,
	...
	ИспользоватьПрямыеПутиФайлов = ИспользоватьПрямыеПутиФайлов Или ФайлОбработки.Существует();
...
Процедура ЗаполнитьДеревоФич(СтруктураПараметров)
	...
	ПодготовитьВыводЛогаВФайл();
	ВывестиВЛогФайл(ПолучитьНаименованиеИВерсиюОбработкиКлиент());
	...
	Если ОписанияФайловШагов.Количество() = 0 Или Объект.ИспользоватьПрямыеПутиФайлов Тогда
	...
	Иначе
		...
		Выполнить("НачатьПомещениеФайлов(
			|	Новый ОписаниеОповещения(""ОбработчикПодключенияШаговИзПереданныхФайловШагов"",
			|		ЭтаФорма, ПараметрыПродолжения),
			|	ПередаваемыеФайлы, , Ложь, ЭтаФорма.УникальныйИдентификатор)");
...
&НаКлиенте
Функция ПроверитьСуществованиеКаталогаПлагина(ДополнительныеПараметры, ПутьКОбработке)
	Перем ПутьФайла;
	...
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		...
		Для каждого Элем Из Объект.КаталогиБиблиотек Цикл
			Ном = Ном + 1;
			Если НЕ ПолучитьЗначениеПоРанееСобраннойИнформации(ДополнительныеПараметры, "КаталогиПлагинов" + Ном, "Существует") Тогда
				БылиОшибки = Истина;
			КонецЕсли;
		КонецЦикла;
	Иначе
		Для каждого Элем Из Объект.КаталогиПлагина Цикл
			ПутьФайла = ПреобразоватьПутьСТочкамиКНормальномуПути(Элем.Значение);
			ФайлПроверкаСуществования = Новый Файл(ПутьФайла);
			Если НЕ ФайлПроверкаСуществования.Существует() Тогда
				ТекстСообщения = "Не найден каталог плагина: %1";
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",Элем.Значение);
				Сообщить(ТекстСообщения);
				БылиОшибки = Истина;
			КонецЕсли;
...
Процедура ДобавитьМассивСостоянийФайлов(Массив)
	...
	Ном = 0;
	Для каждого Элем Из Объект.КаталогиПлагинов Цикл
		Ном = Ном + 1;
		ПутьФайла = ПреобразоватьПутьСТочкамиКНормальномуПути(Элем.Значение);
		ДобавитьДействиеВМассивАсинхроныхСобытий(Массив, ПутьФайла, "КаталогиПлагинов" + Ном, "РаботаСФайлом");
	КонецЦикла;
...
Процедура ПриОткрытииЗавершение(Знач НенужноеЗначение, Знач ДополнительныеПараметры) Экспорт
	...
	Объект.КаталогиПлагинов.ТипЗначения      = Новый ОписаниеТипов("Строка");
...
Процедура АсинхронноПодключитьКонтекстОбработки(Знач ФайлШагов, Знач УниверсальноеПолноеИмяФайлаШага,
	...
	ПередаваемыеФайлы = Новый Массив;
	ПередаваемыеФайлы.Добавить(Вычислить("Новый ОписаниеПередаваемогоФайла(ИмяФайла)"));

	Выполнить("НачатьПомещениеФайлов(
		| Новый ОписаниеОповещения(""ОбработчикАсинхронноПодключитьКонтекстОбработкиПриВыполненииШага"",
		|	ЭтаФорма, ОписаниеФайла),
		| ПередаваемыеФайлы, , Ложь, ЭтаФорма.УникальныйИдентификатор)");
	// из-за асинхронности никакой код вызывать нельзя
...
Процедура ВыполнитьШагЗавершение(ОбработкаТеста, Знач РезультатПрохожденияШага)
	...
	Исключение
		...
		Иначе
			...
			СделатьСообщение(ОписаниеОшибкиШага);
			ИнфоОбОшибке = Неопределено;
			СтрОшибка = СформироватьОписаниеОшибки(ОписаниеОшибкиСтр, ТекШаг.АдресСнипета, ИмяПроцедуры,ИнфоОбОшибке);
...
Функция ПолучитьКонтекстТестаПоИДФайла(Знач ИД, Знач ИмяФайла, Знач ВерсияФайла = Неопределено,
	...
	Если ФормаОбработки = Неопределено Тогда
	 	Файл = Новый Файл(ИмяФайла);
		ПодключитьКонтекстОбработкиКлиент(Файл, ВерсияФайла, АдресВХранилище);
		ФормаОбработки = ТаблицаКонтекстовОбработок[ИД].Обработка;
	КонецЕсли;
...
Функция ВосстановитьНастройкиИзХранилищаПоКлючу(ПрефиксИнструмента, Ключ, ИмяРеквизита)
	...
	Если Ложь
		Или ИмяРеквизита = "КаталогФич"
		Или ИмяРеквизита = "КаталогиБиблиотек"
		Или ИмяРеквизита = "КаталогиПлагинов"
		Тогда
...
Процедура ЗагрузитьПлагины(Знач Перезагрузить = Ложь, Знач ОбработчикПослеЗавершенияЗагрузки = Неопределено)
	...
	КаталогиПлагинов = Новый Массив;
	КаталогиПлагинов.Добавить(ПолучитьПутьКПлагинам());

	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ОбработчикЗавершения", ОбработчикПослеЗавершенияЗагрузки);
	ПараметрыОповещения.Вставить("КаталогиПлагинов",     КаталогиПлагинов);

	ЗагрузитьПлагиныИзКаталогов(Неопределено, ПараметрыОповещения);

КонецПроцедуры

&НаКлиенте
Процедура ЗагрузитьПлагиныИзКаталогов(Результат, ПараметрыОповещения = Неопределено) Экспорт
	
	КаталогПлагинов = ПараметрыОповещения.КаталогиПлагинов[0];
...
&НаКлиенте
Процедура ЗагрузитьПользовательскиеПлагины()
	
	КаталогиПлагинов = Объект.КаталогиПлагинов.ВыгрузитьЗначения();
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("КаталогиПлагинов",     КаталогиПлагинов);
	Если Версия836ИлиВыше Тогда
		ОбработчикПослеЗавершенияЗагрузки = Вычислить("Новый ОписаниеОповещения(""ЗагрузкаКастомныхПалгиновЗавершение"", ЭтаФорма)");
		ПараметрыОповещения.Вставить("ОбработчикЗавершения", ОбработчикПослеЗавершенияЗагрузки);
	КонецЕсли;
	ЗагрузитьПлагиныИзКаталогов(Неопределено, ПараметрыОповещения);
...
&НаКлиенте
Процедура ЗагрузкаКастомныхПалгиновЗавершение(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	
	Отладка("Пользовательские плагины загружены");
...
Процедура ОткрытьИнструмент(Знач ИмяИнструмента, Знач ПутьИнструмента = "", Знач ИмяФормы = "Форма")
	...
	Иначе
		...
		// TODO обработать асинхронность при открытии инструментов
